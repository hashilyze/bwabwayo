    ...
    <div className="relative">
      ...
      <button
        className={`custom-prev-${navigationId} hover:bg-[#343D48] pl-1 z-1 absolute left-0 top-1/2 -translate-y-1/2 w-12 h-12 flex items-center justify-center rounded transition-all duration-200 ${
          isBeginning ? "bg-gray-200 cursor-not-allowed opacity-50" : "bg-white/40 hover:bg-[#343D48] cursor-pointer"
        }`}
        onClick={handlePrev}
      ...
      <button
        className={`custom-next-${navigationId} hover:bg-[#343D48] pr-1 z-1 absolute right-0 top-1/2 -translate-y-1/2 w-12 h-12 flex items-center justify-center rounded transition-all duration-200 ${
          isEnd ? "bg-gray-200 cursor-not-allowed opacity-50" : "bg-white/40 hover:bg-[#343D48] cursor-pointer"
        }`}
        onClick={handleNext}
        disabled={isEnd}
      >
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // 세 API를 병렬로 실행 (더 빠름)
        await Promise.all([getProducts(), getHotKewordProducts(hotKeyword), getVideoCallProducts()]);
      } catch (error) {
    ...
    <div className="py-12">
      ...
      <div className="mb-20">
        <h1 className="text-2xl font-bold mb-5">
          요즘 핫한 키워드 <span className="text-blue-500">{hotKeyword}</span>
        </h1>
