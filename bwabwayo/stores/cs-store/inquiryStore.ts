...
export const useInquiryStore = create<InquiryStore>((set, get) => ({
    ...
    getInquiries: async () => {
        ...
        try {
            // get이라 수정가능
            const response = await useAuthStore.getState().authenticatedFetch(`${baseUrl}/support/inquiries`);
            if (!response.ok) {
    ...
    addInquiry: async (inquiryData: InquiryData) => {
        try{
            ...
            const response = await useAuthStore.getState().authenticatedFetch(`${baseUrl}/support/inquiries/save`, {
                method: 'POST',
